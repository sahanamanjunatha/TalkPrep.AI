...
const InterviewRoom = () => {
  ...
  const fillMockJobDescription = () => {
    let desc = "Seeking a talented software practitioner to help design, test, build and maintain highly reliable scalable features in high performance code environments.";
    const rLower = (role || '').toLowerCase();
    if (rLower.includes('frontend')) {
      ...
      desc = "Seeking a Backend Engineer proficient in Node.js/Express, RESTful API design, PostgreSQL/MongoDB indexing, Redis caching, microservices architecture, and high-concurrency database connection pooling.";
    } else if (rLower.includes('devops')) {
      desc = "Seeking a DevOps / Cloud Engineer skilled in Docker containerization, Kubernetes cluster orchestration, Terraform Infrastructure as Code (IaC), automated CI/CD pipelines, and Prometheus/Grafana telemetry.";
    } else if (rLower.includes('data sci')) {
      desc = "Seeking a Data Scientist experienced in Python ML stacks (PyTorch/Scikit-learn), Feature Engineering, Transformer NLP models, cross-validation, and deploying real-time prediction pipelines.";
    } else if (rLower.includes('data anal')) {
      desc = "Seeking a Data Analyst with robust experience writing complex SQL window queries, building Tableau/PowerBI visualization dashboards, scrubbing raw event tables, and evaluating A/B test statistical significance.";
    } else if (rLower.includes('mobile')) {
      desc = "Seeking a Mobile Engineer experienced in iOS (Swift/UIKit) or Android (Kotlin/Jetpack Compose), MVVM architecture, ARC memory management, offline SQLite caching, and push notification integrations.";
    } else if (rLower.includes('cyber') || rLower.includes('security')) {
      desc = "Seeking a Cybersecurity Specialist skilled in OWASP Top 10 vulnerability remediation, Zero Trust IAM architecture, TLS cryptographic handshakes, SIEM audit logging, and automated SAST/DAST security scans.";
    } else if (rLower.includes('product')) {
      desc = "Seeking a Product Manager to define roadmap priority features, direct cross-functional designer/engineer teams, coordinate sprint planning, and analyze user growth/engagement telemetry metrics using RICE scoring.";
    } else if (rLower.includes('ui') || rLower.includes('ux') || rLower.includes('design')) {
      desc = "Seeking a UI/UX Designer to build accessible component Design Systems, create high-fidelity interactive Figma wireframes, enforce WCAG contrast compliance, and lead continuous user research sessions.";
    } else if (rLower.includes('qa') || rLower.includes('test')) {
      desc = "Seeking a QA Automation Engineer experienced in building Page Object Model (POM) test frameworks in Playwright/Selenium/Cypress, automated API regression testing, and CI/CD quality gate integrations.";
    } else if (rLower.includes('architect')) {
      desc = "Seeking a System Architect to lead enterprise cloud topology designs, evaluate monolithic versus microservice trade-offs, implement CQRS/Event Sourcing patterns, and ensure 99.999% high availability SLAs.";
    }
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2 pt-1 text-[10px] text-slate-400">
                <span className="text-slate-500 my-auto">Popular:</span>
                <button type="button" onClick={() => selectSetupRole('Software Engineer')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">Software Engineer</button>
                ...
                <button type="button" onClick={() => selectSetupRole('Backend Engineer')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">Backend Engineer</button>
                <button type="button" onClick={() => selectSetupRole('DevOps Engineer')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">DevOps Engineer</button>
                <button type="button" onClick={() => selectSetupRole('Data Scientist')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">Data Scientist</button>
                ...
                <button type="button" onClick={() => selectSetupRole('Mobile Engineer')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">Mobile Engineer</button>
                <button type="button" onClick={() => selectSetupRole('Cybersecurity Specialist')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">Cybersecurity</button>
                ...
                <button type="button" onClick={() => selectSetupRole('UI/UX Designer')} className="px-2.5 py-1 rounded-full border border-slate-900 bg-slate-950 hover:border-slate-800 hover:text-white transition-colors cursor-pointer">UI/UX Designer</button>
              </div>
            </div>
