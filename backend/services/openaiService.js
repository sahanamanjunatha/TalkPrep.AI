...
const generateDynamicQuestion = async (role, type, difficulty, index = 1, options = {}) => {
  ...
  if (type === 'Technical' || type === 'System Design') {
    ...
    text = `Given a ${difficulty} array challenge, how would you implement a solution that reduces time complexity from O(N²) to O(N log N) or O(N)?`;
    ...
    text = `Tell me about a time you faced a tight project deadline for a ${role} task. How did you align deliverables and prioritize tasks?`;
    ...
    text = `What interests you about joining ${targetComp}'s team as a ${role}, and how do you align with their engineering standards?`;
